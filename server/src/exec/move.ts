...
export async function moveOne(row: PlanRow): Promise<MoveLogRow> {
  const base: Omit<MoveLogRow, 'status'|'reason'|'strategy'> = { batchId: '', fileId: row.fileId, from: row.from, to: row.to }
  try {
    await mkdir(dirname(row.to), { recursive: true })
    await access(row.to).then(() => { throw new Error('target already exists') }, () => {})
    const strategy = chooseStrategy(row.from, row.to)
    if (strategy === 'rename') {
    ...
    return { ...base, status: 'done', strategy }
  } catch (e) {
